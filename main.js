...
const LedgerAco = require('./LedgerAco')
...
function createWindow () {
  ...
  var iconpath = __dirname + "/client/aco.png";
  ...
  ipcMain.on('ledger', (event, arg) => {
    if(arg.action == "detect"){
      ...
      if(!ledgercomm){
        ...
        aco = new LedgerAco(ledgercomm)
        if(arg.action == "signMessage"){
          aco.signPersonalMessage_async(arg.path, Buffer.from(arg.data).toString("hex")).then(
          ...
          aco.signTransaction_async(arg.path, arg.data).then(
          ...
          aco.getAddress_async(arg.path).then(
          ...
          aco.getAppConfiguration_async().then((result) => {
              console.log(result)
        ...
        var result = {
          connected: false,
          message: "Cannot connect to Aco application"
        }
        event.returnValue = result
      }
    }
  ...
  template = [
    {
      ...
      submenu: [
        {
            label: "About acoclient",
            click: () => openAboutWindow({
              icon_path: __dirname+'/client/aco.png',
              package_json_dir: __dirname,
              copyright: 'Copyright (c) 2017 AriseCoin Foundation, LLC.',
              homepage: 'https://arisecoin.com/',
              bug_report_url: 'https://github.com/arisebank/awallet-core/issues'
            })
        },
        { type: "separator" },
