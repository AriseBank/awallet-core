(function() {
  ...
  angular.module('acoclient.accounts')
  ...
  function AccountService($q, $http, networkService, storageService, ledgerService, gettextCatalog) {
    ...
    function createAccount(passphrase) {
      var deferred = $q.defer();
      var address = aco.crypto.getAddress(aco.crypto.getKeys(passphrase).publicKey, networkService.getNetwork().version);
    ...
    function savePassphrases(address, passphrase, secondpassphrase) {
      var deferred = $q.defer();
      var tempaddress = aco.crypto.getAddress(aco.crypto.getKeys(passphrase).publicKey);
      if (passphrase) {
    ...
    function getTransactionLabel(transaction, recipientAddress = null) {
      ...
      if (recipientAddress && transaction.recipientId == recipientAddress && transaction.type == 0) {
        label = gettextCatalog.getString("Receive Aco");
      }
    ...
    function formatTransaction(transaction, recipientAddress) {
      ...
      if (transaction.recipientId == recipientAddress) {
        ...
        //   transaction.label = gettextCatalog.getString("Receive Aco");
        // }
      }
    ...
    function getFees() {
      var deferred = $q.defer();
      networkService.getFromPeer("/api/blocks/getfees").then(function(resp) {
        if (resp.success) {
    ...
    function verifyMessage(message, publicKey, signature) {
      ...
      var ecpair = aco.ECPair.fromPublicKeyBuffer(new Buffer(publicKey, "hex"));
      var ecsignature = aco.ECSignature.fromDER(new Buffer(signature, "hex"));
    ...
    function signMessage(message, passphrase) {
      ...
      var hash = crypto.createHash('sha256');
      hash = hash.update(new Buffer(message, "utf-8")).digest();
      var ecpair = aco.crypto.getKeys(passphrase);
    ...
    function createTransaction(type, config) {
      ...
      getFees().then(function(fees) {
        if (type == 0) { //send aco
          if (!aco.crypto.validateAddress(config.toAddress, networkService.getNetwork().version)) {
          ...
          if (config.amount + fees.send > account.balance) {
            deferred.reject(gettextCatalog.getString("Not enough ACO on your account ") + config.fromAddress);
            return deferred.promise;
          }

          try {
            var transaction = aco.transaction.createTransaction(config.toAddress, config.amount, config.smartbridge, config.masterpassphrase, config.secondpassphrase);
          } catch (e) {
            deferred.reject(e);
            return deferred.promise;
          }
          ...
          if (config.ledger) {
            ...
            ledgerService.signTransaction(config.ledger, transaction).then(
              function(result) {
                ...
                transaction.id = aco.crypto.getId(transaction);
            ...
            return deferred.promise;
          } else if (aco.crypto.getAddress(transaction.senderPublicKey, networkService.getNetwork().version) != config.fromAddress) {
          ...
          if (account.balance < fees.secondpassphrase) {
            deferred.reject(gettextCatalog.getString("Not enough ACO on your account ") + config.fromAddress + ", " + gettextCatalog.getString("you need at least 5 ACO to create a second passphrase"));
            return deferred.promise;
          }
          try {
            var transaction = aco.signature.createSignature(config.masterpassphrase, config.secondpassphrase);
          } catch (e) {
            deferred.reject(e);
            return deferred.promise;
          }
          ...
          if (account.balance < fees.delegate) {
            deferred.reject(gettextCatalog.getString("Not enough ACO on your account ") + config.fromAddress + ", " + gettextCatalog.getString("you need at least 25 ACO to register delegate"));
            return deferred.promise;
          }
          console.log(config);
          try {
            var transaction = aco.delegate.createDelegate(config.masterpassphrase, config.username, config.secondpassphrase);
          } catch (e) {
            deferred.reject(e);
            return deferred.promise;
          }
          ...
          if (account.balance < fees.vote) {
            deferred.reject(gettextCatalog.getString("Not enough ACO on your account ") + config.fromAddress + ", " + gettextCatalog.getString("you need at least 1 ACO to vote"));
            return deferred.promise;
          }
          try {
            var transaction = aco.vote.createVote(config.masterpassphrase, config.publicKeys.split(","), config.secondpassphrase);
          } catch (e) {
            deferred.reject(e);
            return deferred.promise;
          }
