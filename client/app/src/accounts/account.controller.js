(function() {
  ...
  function AccountController(
    ...
    self.getMarketInfo = function(symbol) {
      changerService.getMarketInfo(symbol, "aco_ACO").then(function(answer) {
        self.buycoin = answer;
      });

      changerService.getMarketInfo("aco_ACO", symbol).then(function(answer) {
    ...
    self.buy = function() {
      ...
      changerService.getMarketInfo(self.selectedCoin, "aco_ACO", self.buyAmount / self.buycoin.rate).then(function(rate) {
        var amount = self.buyAmount / rate.rate;
        if (self.selectedCoin.split("_")[1] == "USD") {
          amount = parseFloat(amount.toFixed(2));
        }
        changerService.makeExchange(self.exchangeEmail, amount, self.selectedCoin, "aco_ACO", self.selected.address).then(function(resp) {
          timeService.getTimestamp().then(
            function(timestamp) {
              ...
              self.exchangeBuy.receiveCurrency = "ACO";
    ...
    var completeExchangeSell = function(timestamp){
      ...
      self.exchangeSell.sendCurrency = "ACO";
      var progressbar = $interval(function() {
    ...
    self.sell = function() {
      if (self.exchangeEmail) storageService.set("email", self.exchangeEmail);
      changerService.makeExchange(self.exchangeEmail, self.sellAmount, "aco_ACO", self.selectedCoin, self.recipientAddress).then(function(resp) {
    ...
    function sendAco(selectedAccount) {
      var passphrases = accountService.getPassphrases(selectedAccount.address);
      var data = {
        ledger: selectedAccount.ledger,
        fromAddress: selectedAccount ? selectedAccount.address : '',
      ...
      $mdDialog.show({
        parent: angular.element(document.getElementById('app')),
        templateUrl: './src/accounts/view/sendAco.html',
        clickOutsideToClose: false,
        preserveScope: true,
        scope: $scope
      });
    ...
    function generateDynamicPalette(callback) {
      ...
      vibrant.from(url).getPalette(function(err, palette) {
        ...
        var isArkJpg = path.basename(url) === 'Aco.jpg';
        ...
        fs.unlink(imagePath, function(err) {
          if (err) {
            ...
            if (image == initialBackground) {
              selectBackground(backgrounds['images']['Aco']);
            } else {
    ...
    function showAccountMenu(selectedAccount) {
      ...
      function answer(action) {
        ...
        } else if (action == gettextCatalog.getString("Send Aco")) {
          sendAco();
