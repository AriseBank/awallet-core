(function() {
  ...
  function LedgerService($q, $http, $timeout, storageService, networkService) {
    ...
    function getBip44Accounts(slip44) {
      ...
      async.whilst(
        ...
        function (next) {
          ...
          if (result.address) {
            result.address = arisejs.crypto.getAddress(result.publicKey);
    ...
    function recoverBip44Accounts(backupLedgerPassphrase, slip44) {
      var hdnode = new arisejs.HDNode.fromSeedHex(bip39.mnemonicToSeedHex(backupLedgerPassphrase));
    ...
    function signTransaction(path, transaction) {
      ...
      ipcRenderer.send('ledger', {
        ...
        data: arisejs.crypto.getBytes(transaction, true, true).toString("hex"),
        path: path
      });
      return deferred.promise;
    }
